...
from airflow.providers.postgres.hooks.postgres import PostgresHook  # Import PostgresHook for PostgreSQL connection
...
from sqlalchemy import create_engine  # Import SQLAlchemy for database interaction
...
POSTGRES_CONN_ID_local = 'postgres_default1'  # Connection ID for PostgreSQL
...
def get_top_countries():
    ...
    if not table:  # If no table is found, return an empty list
        return []
    rows = table.find("tbody").find_all("tr")[:1]  # Get the top 10 countries for demonstration
    return [
...
# Define the DAG
with DAG(
    dag_id='updated_weather_etl_pipeline',  # The unique ID of the DAG
    ...
    @task()
    def load_weather_data_to_postgres(transformed_file):
        """Load transformed weather data into PostgreSQL."""
        
        # Create a PostgreSQL hook to manage database connections
        pg_hook = PostgresHook(postgres_conn_id=POSTGRES_CONN_ID_local)
        
        # Step 1: Establish a connection to the database
        conn = pg_hook.get_conn()
        cursor = conn.cursor()  # Create a cursor to execute SQL queries
        
        # Step 2: Create the weather_forecast table if it doesn't already exist
        cursor.execute("""
        ...
        # Step 4: Insert the transformed data into the table
        for _, row in df.iterrows():
            cursor.execute("""
            INSERT INTO weather_forecast (city, date, hour, temperature, windspeed, winddirection, weathercode)
        ...
        # Step 6: Close the cursor and connection to PostgreSQL to free resources
        cursor.close()
        conn.close()
    ...
    load_weather_data_to_postgres(transformed_file)  # Load the transformed data into PostgreSQL
